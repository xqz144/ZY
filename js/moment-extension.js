...
(function() {
    ...
    // 评论自动回复：梦角对"我"的评论自动回复
    // 注意：moments.js 的评论是扁平结构 m.comments[]，字段为 name/text/replyTo/sticker
    ...
    function genAutoReply(post, targetComment) {
        ...
        // 追加到 post.comments（扁平结构），字段名与 moments.js 渲染一致
        const newComment = {
            name: partnerName,
            text: pickText || '嗯？',
            replyTo: targetComment.name || undefined,
            sticker: undefined,
